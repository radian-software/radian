...
(when (member 'magit radon-packages)
...
(when (member 'clojure-mode radon-packages)
  ...
  (eval-after-load 'clojure-mode
    (lambda ()
      ...
      ;; We can't use define-clojure-indent here, due to a perverse threefold
      ;; conspiracy perpetrated by the Gilardi scenario, recursive
      ;; macroexpansion, and dash.el.
      ;; See http://emacs.stackexchange.com/q/26261/12534
      (dolist (spec '((-> 1)
                      (->> 1)
                      ...
                      ;; A hack within a hack! Hack-ception...
                      (:import 0)
                      (:overall-average 0)
                      (:require 0)
                      (:use 0)))
