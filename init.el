          ...
          (when-let ((link-target
                      ...
                      (file-symlink-p
                       (or
                        ...
                        load-file-name
                        ;; Otherwise, that's nil and the filename is
                        ;; in `buffer-file-name' (this will happen if
                        ;; you `eval-buffer' for example).
                        buffer-file-name))))
            ;; We identify a directory as the Radian repository by the
            ;; existence of a "radian-emacs" folder inside it. Note
            ;; that the previous check does disallow copying the
            ;; init-file and radian-emacs folder into ~/.emacs.d,
            ;; which is fine as that is a ridiculous use case.
...
code.")
        ...
        ;; Allow to disable local customizations with a command-line
        ;; argument.
        (if (member "--no-local" command-line-args)
            (progn
              ;; Make sure to delete --no-local from the list, because
              ;; otherwise Emacs will issue a warning about the
              ;; unknown argument.
          ...
          ;; system has not yet been loaded. See [1] for the hack used
          ;; to disable eager macroexpansion.
          ;;
        ...
        (let ((preloaded-features
          ...
          ;; packages. (Packages installed interactively do not belong
          ;; to either `radian' or `radian-local', and should not be
          ;; written to either lockfile.)
